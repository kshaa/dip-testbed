...
import iotfrisbee.database.services.{SoftwareService, UserService}
import iotfrisbee.domain.SoftwareId
...
class SoftwareController(
  ...
  def createSoftware: Action[MultipartFormData[Files.TemporaryFile]] =
    Action(parse.multipartFormData)(withRequestAuthn(_)((request, maybeUser) => {
      val authWithSoftwareData = EitherT.fromEither[IO](maybeUser
        .leftMap(databaseErrorResult)
        .flatMap(_.toRight(authorizationErrorResult))
        .flatMap(user =>
          (request.body.file("software"),
            request.body.dataParts.get("name").flatMap(_.headOption))
            .tupled
            .toRight(Failure("Request must contain 'software' file and 'name' field").withHttpStatus(BAD_REQUEST))
            .flatMap { case (data, name) =>
              Option.when(data.ref.length() < maxBytes)(data).toRight(
                Failure("Request too large").withHttpStatus(BAD_REQUEST)
              ).map(_ => (data, name, user))
            }))

      val authWithSoftwareBytes = authWithSoftwareData.flatMap { case (data, name, user) =>
        Try {
          ...
          .toEitherT[IO].map((_, name, user))
      }

      val pipeline = authWithSoftwareBytes.flatMap { case (bytes, name, user) =>
  ...
  def getSoftwareMetas: Action[AnyContent] =
    IOActionAny { _ =>
      EitherT(softwareService.getSoftwareMetas).leftMap(databaseErrorResult)
      ...
      EitherT(softwareService.getSoftware(softwareId))
        ...
        .map(software => {
          ...
          Ok.sendFile(
            tempFile.jfile,
            inline = true,
            fileName = _ => software.meta.name.some,
