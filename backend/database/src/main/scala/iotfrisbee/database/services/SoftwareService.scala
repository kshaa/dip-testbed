...
import iotfrisbee.database.catalog.SoftwareCatalog.{SoftwareRow, SoftwareTable}
...
import iotfrisbee.domain.{Software, SoftwareId, SoftwareMeta, UserId}
...
class SoftwareService[F[_]: Async](
  ...
  def createSoftware(
    ...
    softwareCreation
      .tryRunDBIO(dbDriver)
      .map(dbioAction => dbioAction.map(softwareId => softwareId.map(_ =>
        SoftwareMeta(SoftwareId(row.id), UserId(row.ownerId), row.name))))
  }
  ...
  def getSoftware(id: SoftwareId): F[DatabaseResult[Option[Software]]] =
    SoftwareQuery
      ...
      .headOption
      .map(_.map(row =>
        Software(SoftwareMeta(SoftwareId(row.id), UserId(row.ownerId), row.name), row.content)
      ))
      .tryRunDBIO(dbDriver)
  ...
  def getSoftwareMeta(id: SoftwareId): F[DatabaseResult[Option[SoftwareMeta]]] =
    SoftwareQuery
      ...
      .result
      .headOption
      .map(_.map { case (id, ownerId, name) =>
